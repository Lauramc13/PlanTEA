...
class AdaptadorCuadernoActivity(var listaPictogramas: ArrayList<Pictograma>?, private val listener: OnItemSelectedListener?) : RecyclerView.Adapter<AdaptadorCuadernoActivity.ViewHolderPictogramas>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolderPictogramas {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.item_cuaderno, null, false)
    ...
    inner class ViewHolderPictogramas(itemView: View) : RecyclerView.ViewHolder(itemView), View.OnClickListener {
        ...
        init {
            titulo = itemView.findViewById<View>(R.id.id_texto_item) as TextView
            imagen = itemView.findViewById<View>(R.id.id_imagen_item) as ImageView
