...
class GestionPlanificaciones : Serializable {
    ...
    fun insertarPictogramaPlan(actividad: Activity?, pictogramas: java.util.ArrayList<Pictograma>, titulo: String?): Boolean {
        ...
        for (i in pictogramas.indices) {
            resultado = conectorBD!!.insertarPictogramaPlan(pictogramas[i]?.titulo, pictogramas[i]?.imagen, pictogramas[i]!!.categoria, id_plan)
        }
        conectorBD!!.cerrar()
        return resultado!!
    }
    ...
    fun actualizarPlanificacion(actividad: Activity?, id_plan: Int, nombre: String?, pictogramas: ArrayList<Pictograma>) {
        ...
        for (i in pictogramas.indices) {
            conectorBD!!.insertarPictogramaPlan(pictogramas[i]?.titulo, pictogramas[i]?.imagen, pictogramas[i]!!.categoria, id_plan)
        }
        conectorBD!!.cerrar()
    }
