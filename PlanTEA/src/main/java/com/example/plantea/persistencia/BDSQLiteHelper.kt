...
/*
    Clase que se encarga de la creación de la base de datos. Se poblara con datos iniciales de los pictogramas por
    defecto y las categorias por defecto.
 */
...
class BDSQLiteHelper(contexto: Context?, nombreBD: String?, factory: CursorFactory?, versionBD: Int) : SQLiteOpenHelper(contexto, nombreBD, factory, versionBD) {
    ...
    private var sqlPictogramaEvento = "CREATE TABLE PictogramaEvento (id INTEGER PRIMARY KEY AUTOINCREMENT, posicion INTEGER, titulo_alt TEXT, imagen_alt BLOB, duracion TEXT, historia TEXT, is_imprevisto INTEGER, id_picto_entre INTEGER, id_evento INTEGER, id_pictograma INTEGER, FOREIGN KEY (id_evento) REFERENCES Evento(id), FOREIGN KEY (id_pictograma) REFERENCES RelacionPictogramaPlan(id_pictograma))"
    ...
    /**
     * @function que inserta los datos iniciales en la base de datos
