...
class EventosFragment : Fragment(), AdaptadorEvento.OnItemSelectedListener {
    ...
    private fun iniciarAdaptadorEvento() {
        ...
        viewModel.eventosDia = viewModel.gEvento.obtenerEventos(userId!!, actividad, CalendarioUtilidades.fechaSeleccionada)
    ...
    override fun viewClick(posicion: Int) {
        ...
        val eventoVisible = viewModel.idUsuario.let { viewModel.gEvento.comprobarEventoVisible(it, fecha, actividad) }

        if (viewModel.eventosDia[posicion].visible == 1) {
            viewModel.gEvento.cambiarVisibilidad(actividad, 0, viewModel.eventosDia[posicion].id)
        } else {
            viewModel.gEvento.cambiarVisibilidad(actividad, 0, eventoVisible)
            viewModel.gEvento.cambiarVisibilidad(actividad, 1, viewModel.eventosDia[posicion].id)
        }
    ...
    override fun viewEventClick(posicion: Int) {
        val pictogramas = viewModel.gPlan.obtenerPictogramasPlanificacionEvento(actividad, viewModel.eventosDia[posicion].idPlan, viewModel.eventosDia[posicion].id, Locale.getDefault().language, viewModel.idUsuario)
    ...
    override fun editEvento(posicion: Int) {
        val evento = viewModel.gEvento.obtenerInfoEvento(viewModel.eventosDia[posicion].id, actividad)
