...
class EventosPlanificadorActivity : AppCompatActivity(), AdaptadorListaEventos.OnItemSelectedListener, AdaptadorPictogramasEventos.OnItemSelectedListener {
    ...
    private val viewModelCalendario : CalendarioViewModel by viewModels()
    ...
    private fun iniciarListaPlanificaciones() {
        ...
        viewModel.eventos = viewModel.gEvento.obtenerTodosEventos(viewModel.idUsuario, this)
...
//        itemTouchHelper.attachToRecyclerView(recyclerPictogramas)
    ...
    override fun verEvento(posicion: Int, context: Context) {
        val gPlan = GestionPlanificaciones()
        val pictogramas = gPlan.obtenerPictogramasPlanificacionEvento(this, viewModel.eventos[posicion].idPlan, viewModel.eventos[posicion].id, Locale.getDefault().language, viewModel.idUsuario)
    ...
    override fun cambiarVisibilidadEvento(posicion: Int, context: Context) {
        val eventoVisible = viewModel.idUsuario.let { viewModel.gEvento.comprobarEventoVisible(it, viewModel.eventos[posicion].fecha.toString(), this) }

        if (viewModel.eventos[posicion].visible == 1) {
            viewModel.gEvento.cambiarVisibilidad(this, 0, viewModel.eventos[posicion].id)
        } else {
            viewModel.gEvento.cambiarVisibilidad(this, 0, eventoVisible)
            viewModel.gEvento.cambiarVisibilidad(this, 1, viewModel.eventos[posicion].id)
        }
    ...
    override fun exportarEvento(posicion: Int, context: Context) {
        ...
        if (resolvedActivities.isNotEmpty()) {
            val chooserIntent = Intent.createChooser(intent, getString(R.string.toast_exportar_evento))
    ...
    private fun configurarEventos() {
        ...
        viewModel.eventos.addAll(viewModel.gEvento.obtenerTodosEventos(viewModel.idUsuario, this))
    ...
    override fun entretenimientoSeleccionado(posicion: Int, context: Context) {
        ...
        val listaActividades = gActividad.getAllActividadesAsPictogramas(viewModel.idUsuario, this)
        recyclerActividad(recyclerActividad, dialogEntretenimiento, listaActividades, idPictoEntretenimiento)
