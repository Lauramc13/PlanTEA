...
class SemanaActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(savedInstanceState == null){
            viewModel.configuration = gSemana.obtenerconfig(viewModel.idUsuario, this)
            viewModel.colorsHeader = gSemana.obtenerColoresHeader(viewModel.idUsuario,this)
        }
...
//    }
    ...
    fun observers() {
        ...
        viewModel.mdDiaClicked.observe(this) {
            ...
            val eventoSelected = gEvento.obtenerInfoEvento(idEvento!!.toInt(), this)

            val gPlan = GestionPlanificaciones()
            val pictogramas = gPlan.obtenerPictogramasPlanificacionEvento(this, eventoSelected.idPlan,eventoSelected.id, Locale.getDefault().language, viewModel.idUsuario)
        ...
        buttonGuardar.setOnClickListener {
            for(i in 0..6){
                if(viewModel.week[i].imagen != null || viewModel.week[i].color != null) {
                    ...
                    gSemana.guardarSemana(viewModel.idUsuario, imageBlob, viewModel.week[i].color, viewModel.week[i].dia, viewModel.week[i].idEvento, this)
                }

               if(viewModel.week[i].imagen == null){
                   gSemana.borrarImagen(viewModel.idUsuario, viewModel.week[i].dia!!, this)
                    viewModel.week[i].idEvento = null
               }

                if(viewModel.week[i].color == null){
                    gSemana.borrarColor(viewModel.idUsuario, viewModel.week[i].dia!!, this)
                }
            }

            gSemana.guardarConfiguracionWeek(viewModel.idUsuario, viewModel.configuration, this)
            gSemana.guardarColorsHeader(viewModel.idUsuario, viewModel.colorsHeader!!, this)
