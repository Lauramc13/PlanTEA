...
class CrearPlanActivity : AppCompatActivity(){
    ...
    private fun clickGuardarPicto(){
        if(viewModel.listaPlanificacion.isEmpty()){
            ...
            btnGuardar?.setOnClickListener {
                val listaTitulos = viewModel.gPlanificacion.obtenerTitulosPlanificaciones(viewModel.idUsuario, this)
                ...
                if (viewModel.opcionEditar) {
                    ...
                    viewModel.gPlanificacion.actualizarPlanificacion(this@CrearPlanActivity, viewModel.idUsuario, idPlan, title, viewModel.listaPlanificacion)
                    Toast.makeText(this, R.string.toast_plan_actualizado, Toast.LENGTH_SHORT).show()

                } else {
                    idPlan = viewModel.gPlanificacion.crearPlanificacion(this@CrearPlanActivity,  viewModel.idUsuario, title)
                    val creada = viewModel.gPlanificacion.addPictogramasPlan(idPlan, viewModel.idUsuario, viewModel.listaPlanificacion, this@CrearPlanActivity)
    ...
    private fun createPickMedia() {
        ...
        viewModelTraductor.pickMedia = registerForActivityResult(ActivityResultContracts.PickVisualMedia()) { uri: Uri? ->
            if (uri != null) {
                val inputStream = this.contentResolver?.openInputStream(uri)
                viewModelTraductor.listaPictogramas[viewModelTraductor.posicionSelected].imagen = BitmapFactory.decodeStream(inputStream)
                viewModelTraductor.listaPictogramas[viewModelTraductor.posicionSelected].idAPI = -1
                viewModelTraductor.adaptador.notifyItemChanged(viewModelTraductor.posicionSelected)
            } else {
                Toast.makeText(this, R.string.toast_no_imagen_seleccionada, Toast.LENGTH_SHORT).show()
            }
        }
