...
class AdaptadorCategorias(var listaCategorias: ArrayList<Categoria>?, private val listener: OnItemSelectedListener?) : RecyclerView.Adapter<AdaptadorCategorias.ViewHolderCategorias>() {
    ...
    override fun onBindViewHolder(holder: ViewHolderCategorias, position: Int) {
        ...
        holder.titulo.text = listaCategorias!![position].nombre
        ...
        if(position == listaCategorias!!.size-1){
           ...
           holder.imagen.setImageBitmap(listaCategorias!![position].imagen)
        }
        ...
        if(listaCategorias!![position].color == "default") {
            ...
            holder.card.setCardBackgroundColor(CommonUtils.getColor(context, listaCategorias!![position].color!!))
    ...
    inner class ViewHolderCategorias(itemView: View) : RecyclerView.ViewHolder(itemView) {
        var titulo: TextView = itemView.findViewById<View>(R.id.lbl_titulo) as TextView
        var imagen: ImageView = itemView.findViewById<View>(R.id.img_categoria) as ImageView
        var card: CardView = itemView.findViewById<View>(R.id.categoria) as CardView
        var borrar: ImageView = itemView.findViewById<View>(R.id.btn_borrarPicto) as ImageView
        ...
        init {
            itemView.setOnClickListener {
                ...
                if(posicion == listaCategorias!!.size-1){
                    //pass the activity
                    listener?.addCategoria(itemView, itemView.context as Activity)
                } else {
                    listener?.onItemSeleccionado(listaCategorias!![posicion].id, itemView.context)
                }
            }

            borrar.setOnClickListener{
                val posicion = bindingAdapterPosition
                listener?.borrarCategoria(posicion, listaCategorias!![posicion].id, itemView)
            }
        }
    }
}